...
void main() {
  ...
  group("mergeUpdate -", () {
    ...
    test("insert a new row", () async {
      ...
      await db.mergeUpdate(db.departuresTable, departure, (d) =>
    ...
    test("update a row with non-null fields", () async {
      ...
      await db.mergeUpdate(db.departuresTable, updatedDeparture, (d) =>
      d.runRef.equals(departure.runRef.value) &
      d.stopId.equals(departure.stopId.value) &
      d.routeId.equals(departure.routeId.value) &
      d.directionId.equals(departure.directionId.value),
      ...
      expect(result!.runRef, updatedRunRef, reason: "Non-nullable field should be updated");
      expect(result.hasLowFloor, updatedHasLowFloor, reason: "Nullable field should be updated");
    ...
    test("update a row's updatable fields", () async {
      ...
      expect(result!.hasLowFloor, hasLowFloor, reason: "Updatable field shouldn't change due to new absent value");
      expect(result.hasAirConditioning, updatedHasAirConditioning, reason: "Updatable field should change due to new non-null value");
    });
    ...
    test("update a row's overwrite fields", () async {
      ...
      String runRef = "0";
      int stopId = 1, routeId = 2, directionId = 3;
      DateTime? scheduledDeparture = DateTime(1), estimatedDeparture = DateTime(2);
      ...
      // 2. Update departure
      DateTime? updatedEstimatedDeparture = null,  updatedScheduledDeparture = DateTime(3);
      final updatedDeparture = db.createDepartureCompanion(
        runRef: runRef,
        stopId: stopId,
        routeId: routeId,
      ...
      expect(result!.scheduledDepartureUtc, updatedScheduledDeparture, reason: "Overwrite field should change to new non-null value");
      expect(result.estimatedDepartureUtc, updatedEstimatedDeparture, reason: "Overwrite field should change to new null value");
    });
  });
}