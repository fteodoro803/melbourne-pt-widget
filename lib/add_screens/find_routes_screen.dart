...
enum BusFilter {
  ...
  const BusFilter({required this.name, required this.id});
...
enum VLineFilter {
  ...
  const VLineFilter({required this.name, required this.id});
...
class _FindRoutesScreenState extends State<FindRoutesScreen> {
  ...
  int extractNumericPart(String str) {
    ...
    if (match != null) {
      return int.parse(
          match.group(0)!); // Convert the matched number to an integer
    }
    ...
    setState(() {
      _filteredRoutesBySearch = _filteredRoutes.where((route) {
        return route.name.toLowerCase().contains(searchTerm) ||
            (route.number.isNotEmpty &&
                route.number.toLowerCase().contains(searchTerm));
      }).toList();
    });
  }
          ...
          Row(
            children: [
              ...
              Wrap(
                spacing: 5.0,
                children:
                    busFilters.entries.map((MapEntry<BusFilter, bool> filter) {
                  return FilterChip(
        ...
        if (_transportTypeFilters['vLine'] == true) ...[
          Row(
            children: [
              ...
              Wrap(
                spacing: 5.0,
                children: vLineFilters.entries
                    .map((MapEntry<VLineFilter, bool> filter) {
                  return FilterChip(
                      label: Text(filter.key.name),
                      selected: filter.value,
                      onSelected: (bool selected) {
                ...
                itemBuilder: (context, index) {
                  ...
                  return Card(
                      ...
                      child: ListTile(
                          ...
                          leading:
                              RouteLabelContainer(route: route, textSize: 16),
                          title: routeType != "train"
                              ? Text(route.name,
                                  style: TextStyle(fontSize: 15, height: 1.3))
                              : null,
                          subtitle: routeType != "tram" && routeType != "train"
                              ? Text(route.gtfsId)
                              : null,
                          trailing: Icon(Icons.arrow_forward_ios, size: 14),
                          onTap: () => Get.to(
                                () => SearchScreen(
                                    route: route, enableSearch: false),
